...
pub trait EntryNumberExistenceChecker: Send + Sync {
...
#[allow(async_fn_in_trait)]
pub trait VoucherNumberGenerator: Send + Sync {
...
mod tests {
    ...
    impl EntryNumberExistenceChecker for MockEntryNumberChecker {
        async fn exists(&self, entry_number: &EntryNumber) -> DomainResult<bool> {
    ...
    fn test_validate_entry_number_uniqueness_success() {
        ...
        let result = tokio_test::block_on(JournalEntryService::validate_entry_number_uniqueness(
            &new_number,
            &checker,
        ));
    ...
    fn test_validate_balance_success() {
        ...
        let result = JournalEntryService::validate_balance(&lines);
        assert!(result.is_ok());
    }
    ...
    fn test_create_reversal_lines() {
        ...
        let reversed = JournalEntryService::create_reversal_lines(&original_lines).unwrap();
    ...
    mod property_tests {
        ...
        fn valid_amount_strategy() -> impl Strategy<Value = f64> {
            (0i64..=1_000_000_00i64).prop_map(|cents| cents as f64 / 100.0)
        }
        ...
        proptest! {
            ...
            fn prop_reversal_preserves_line_count(line_count in 1usize..10usize) {
                ...
                let reversed = JournalEntryService::create_reversal_lines(&lines).unwrap();
            ...
            fn prop_reversal_inverts_sides(amount in valid_amount_strategy()) {
                ...
                let reversed = JournalEntryService::create_reversal_lines(&original).unwrap();
            ...
            fn prop_double_reversal_returns_original(amount in valid_amount_strategy()) {
                ...
                let reversed_once = JournalEntryService::create_reversal_lines(&original).unwrap();
                let reversed_twice = JournalEntryService::create_reversal_lines(&reversed_once).unwrap();
