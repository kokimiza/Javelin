...
impl IfrsValuationPageState {
    pub fn new(controllers: &Controllers) -> Self {
        ...
        let mut page = IfrsValuationPage::new();
        page.set_loading();

        let controller = std::sync::Arc::clone(&controllers.batch_history);
        let batch_type = "IfrsValuation".to_string();
        ...
        loop {
                    ...
                    event::read().map_err(crate::error::AdapterError::EventReadFailed)?
            {
                ...
                match key.code {
                    ...
                    KeyCode::Char('e') => {
                        return Ok(NavAction::Go(Route::IfrsValuationExecution));
                    }
...
impl Drop for IfrsValuationPageState {
    fn drop(&mut self) {
